...
public class myTrips extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ridesDatabaseReference = FirebaseDatabase.getInstance().getReference("Rides");
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        reqList = new ArrayList<>();
        getRequests= getIntent();
        requests = getRequests.getStringArrayListExtra("requestList");
        email = getRequests.getStringExtra("email");
        String tripType = getRequests.getStringExtra("TripType");
        ridesDatabaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                // Handle rides data
                for (DataSnapshot ridesSnapshot : snapshot.getChildren()) {
                    Route route = ridesSnapshot.getValue(Route.class);
                    String newStatus = getRouteNewStatus(route.Status,route.Date,route.Time,route);
                    route.Status= newStatus;
                    if(tripType.equals("Upcoming")){
                        if (route != null && route.Ride_ID != null && requests.contains(route.Ride_ID) && ((route.Status).equals("Confirmed"))) {
                            reqList.add(route);
                        }
                    }else if(tripType.equals("Pending")){
                        if (route != null && route.Ride_ID != null && requests.contains(route.Ride_ID) && ((route.Status).equals("Not Confirmed"))) {
                            reqList.add(route);
                        }
                    }
                    else if(tripType.equals("Done")){
                        if (route != null && route.Ride_ID != null && requests.contains(route.Ride_ID) && ((route.Status).equals("Done"))) {
                            reqList.add(route);
                        }
                    }
                    else if(tripType.equals("InProgress")){
                        if (route != null && route.Ride_ID != null && requests.contains(route.Ride_ID) && ((route.Status).equals("In Progress"))) {
                            reqList.add(route);
                        }
                    }
                    else if(tripType.equals("Cancelled")){
                        if (route != null && route.Ride_ID != null && requests.contains(route.Ride_ID) && ((route.Status).equals("Cancelled"))) {
                            reqList.add(route);
                        }
                    }
                    Log.d("MyTrips", "2Requests data: " + requests.toString());
    ...
    // Method to update the adapter when both requests and reqList are ready
    private void updateAdapter() {
        adapterRequest = new AdapterRequest(this, reqList, email);
